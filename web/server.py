...
def _pick_folder():
    """Open a native OS folder picker dialog (tkinter)."""
...
async def list_project_files():
    project = get_project_path()
    if not project:
        return JSONResponse({"error": "No project set"}, status_code=400)

    config = get_config()
    source_dir = config.project_source_dir.strip("/").strip("\\")
    scan_dir = os.path.join(project, source_dir)
    if not os.path.isdir(scan_dir):
        return JSONResponse({"error": f"No {source_dir}/ directory"}, status_code=400)

    tree = _build_file_tree(scan_dir, project)
    return {"tree": tree}


def _build_file_tree(directory: str, project_root: str) -> list:
    """Build a nested file tree for the frontend."""
    entries = []
    try:
    ...
    for item in items:
        ...
        if os.path.isdir(full):
            children = _build_file_tree(full, project_root)
            entries.append({
                "name": item,
                "path": rel,
                "type": "directory",
...
async def read_file(path: str):
    ...
    config = get_config()
    source_dir = config.project_source_dir.strip("/").strip("\\")
    if not normalized.startswith(os.path.normpath(os.path.join(project, source_dir))):
